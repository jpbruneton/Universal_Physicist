...
def fetch_and_download_topic(
    ...
    """
    Single-topic INSPIRE sweep (top-cited) + download abstracts via arXiv.
    Used by main.py when config papers.inspire is true.
    """
    print(f"INSPIRE (single topic): {topic[:120]}...")
    ids = fetch_topic(topic, max_results, min_citations, dry_run=False)
    existing = load_existing_arxiv_ids()
    new_ids = [i for i in ids if i not in existing]
    print(f"  -> {len(ids)} INSPIRE hits, {len(new_ids)} not yet in library")
    if not new_ids:
        return
    batch_size = 20
