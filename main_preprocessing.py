...
Steps (all automatic):
  1. Core curated papers         — arxiv_downloader  --core
  2. INSPIRE-HEP top-cited sweep — inspire_downloader (all topics)
  3. Semantic Scholar sweep      — semantic_scholar   (QIT / foundations)
  4. Full-text PDF extraction    — pdf_reader         (for any downloaded PDFs)
  5. AI preprocessing            — preprocess_papers  (summaries, keywords, exclusions)

Usage:
  py -3 main_preprocessing.py                    # full pipeline, abstracts only
  py -3 main_preprocessing.py --pdf              # also download full PDFs (slow)
  py -3 main_preprocessing.py --min-citations 30 # stricter citation filter
  py -3 main_preprocessing.py --quick            # core papers only + preprocess
  py -3 main_preprocessing.py --skip-inspire     # skip INSPIRE (use if offline)
  py -3 main_preprocessing.py --skip-semantic    # skip Semantic Scholar
  py -3 main_preprocessing.py --force            # re-preprocess everything
  py -3 main_preprocessing.py --status           # show current library status
...
def run_pipeline(
    ...
    if not ANTHROPIC_API_KEY:
        ...
        print("  Set it and run:  py -3 preprocess_papers.py")
    else:
